...
- **Cap-aware offer sizing**: offer sizes are derived from the platform's sats cap at the current spot price, so they stay maximal as the BTC price moves
...
│   ├── offer_sizing.py    # Offer sizes derived from the platform's sats cap
...
- **Auto buy-escrow**: Interval, offer sizing (`size_mode`, `cap_fractions`, `max_offer_sats`), fixed premium, excluded methods
...
- **Offer sizes derived from the sats cap**: The platform caps an offer in *sats* while the rotation is configured in *fiat*. A static fiat list silently loses its largest entries when BTC falls and leaves headroom unused when it rises. Expressing sizes as fractions of the cap keeps the largest offer at ~98% of what the platform allows at any price. Larger offers also mean fewer bank transfers for the same volume, which matters beyond fees.
