"""Core trading engine: the periodic loop that drives every platform.

The engine owns the tick: it polls contracts, matches trade requests, decides
when to open offers, funds escrows and records completed trades.

Note on this public repository
------------------------------
The operative trading logic — offer creation and sizing decisions, escrow
funding coordination, trade-request matching and auto-accept, and premium
maintenance on stale offers — is not published here. Those methods are kept as
documented signatures so the architecture, the control flow and the module
boundaries stay readable, but they raise NotImplementedError.

Everything else in this repository is complete and real, including the taproot
escrow implementation, BIP32 key derivation, offer sizing arithmetic, the
platform and exchange clients, and the data model. The intent is to show how
the system is built without shipping a bot that runs unattended against a live
marketplace and real money.
"""
...
class TradingEngine:
    ...
    def _rotation_amounts(self, cfg):
        ...
        if cfg.get("size_mode", "fixed") != "cap_fraction":
            return configured
        try:
            spot = SpotPriceProvider.get_spot_eur()
        except Exception:
            return configured
    ...
    def _auto_buy_escrow_check(self):
        """Decide whether to open a new offer this cycle, and at which size.

Applies the size rotation, the concurrency and spacing limits, and the
available fiat balance, then reserves the sats the resulting offer will draw
before handing off to the buy pipeline.

        Not included in the public repository — see the note at the top of this
        module.
        """
        raise NotImplementedError(
            "_auto_buy_escrow_check is not part of the public repository")

    def _auto_fund_wallet_check(self):
        """Move confirmed hot-wallet sats into escrows awaiting funding.

Accounts for sats already reserved by other pending offers so two offers can
never be funded from the same UTXOs, and caps the amount at the marketplace
limit.
        ...
        """
        raise NotImplementedError(
            "_auto_fund_wallet_check is not part of the public repository")
    ...
    def _create_offers(self, name, platform):
        """Create the sell offers for a platform and register their escrows.

        Not included in the public repository — see the note at the top of this
        module.
        """
        raise NotImplementedError(
            "_create_offers is not part of the public repository")
        # Offers are created manually via /buy_escrow — no auto-create limit
    def _fund_escrows(self, name, platform):
        """Fund the escrows of offers that are waiting for coins.

        Not included in the public repository — see the note at the top of this
        module.
        """
        raise NotImplementedError(
            "_fund_escrows is not part of the public repository")
    def _check_matches(self, name, platform):
        """Poll incoming trade requests and auto-accept the ones that qualify.

Resolves the authoritative live-offer list, filters out offers that are
terminal or already contracted, matches each request against the published
payment data, and accepts under the configured constraints.

        Not included in the public repository — see the note at the top of this
        module.
        """
        raise NotImplementedError(
            "_check_matches is not part of the public repository")
    ...
    def _check_stale_offers(self, name, platform):
        """Reduce the premium on offers that have gone unmatched.

Operates on the live offer list rather than local state, so manually created
offers are covered too.

        Not included in the public repository — see the note at the top of this
        module.
        """
        raise NotImplementedError(
            "_check_stale_offers is not part of the public repository")
