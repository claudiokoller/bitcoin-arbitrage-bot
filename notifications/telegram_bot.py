...
class TelegramBot:
    ...
    def _get_sepa_payment_data(self, pconfig, sepa_idx):
        ...
        for method, val in raw.items():
            ...
            # Peach hashes each field's value on its own (lowercased). For revolut/wise the hashed
            # field is `userName` -> hash the raw userName string directly (NOT a JSON blob).
